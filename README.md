...
An AUC (a standard measure of how well a model separates two groups, from 0.5 = random guessing to 1.0 = perfect separation) of 0.626 is a modest, genuine, statistically confirmed signal — not a diagnostic tool, and I want to be precise about that rather than let a table of numbers imply more than it should. What makes it a strong result on its own terms is that it's *real*: it shows up again in a completely separate group of patients, tested against a bar — independent external validation — that most exploratory transcriptomics analyses never even attempt.
...
I initially got a near-perfect classifier separating early- and late-onset colorectal cancer patients by gene expression. That result turned out to be a technical artifact — the patient groups had been sequenced under different lab conditions — so I identified it, rejected it, and rebuilt the study on a cleaner dataset. There, a gene-level model found a real signal that held up on an independent cohort, while a pathway-based approach and a previously published gene panel did not — a clean, decisive comparison. The genes driving the working model point toward the tumor's surrounding tissue more than the tumor cells themselves, a precise and important finding given the type of data involved. The full reasoning — including exactly how that early confounded result was caught and corrected — is documented throughout this repository as a core part of the project, not tucked away.
